...
//  наковальни
        for(int j = 0; j < this->coordList->value(1)->count() ; j++){
            ...
            int coords[4];
            for(int k = 0; k < 4; k++) coords[k] = this->coordList->value(1)->value(j).split(',').value(k).toInt();
            vAnvils[j]->setGeometry(coords[0],coords[1],coords[2],coords[3]);
        }
//  тумблеры
        for(int j = 0; j < this->coordList->value(3)->count() ; j++){
            vTumblers[j] = new VariablePicture(this, "Tum", this->deskNumber, 0);
            int coords[4];
